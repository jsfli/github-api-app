...
function doRequest(name, index, callback) {
    ...
    if (this._config.registry.search[index].indexOf("http") < 0) {

        var fileName = this._config.registry.search[index];
        //mac osx issues here
        //if (!fs.exists(fileName)) return callback(createError(fileName + " not found!", 'EINREG'));
        fs.readFile(fileName, function(err, data) {
            ...
            for(var i = 0; i < registry.length; i++) {
                ...
                if (item.name.toLowerCase().indexOf(name.toLowerCase()) > -1) {
                    found = true;
                    console.log(name + " @ " + item.url);
                    callback(null, item);
                    break;
                }
            }
            if (!found) console.log(name + " not found in " + fileName);
        });
